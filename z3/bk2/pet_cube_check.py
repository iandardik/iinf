...
cube1 = init
cube2 = And(turn == p2, flag(p1) == False, flag(p2) == True, pc(p1) == 1, pc(p2) == 4)
cube3 = And(turn == p2, flag(p1) == False, flag(p2) == True, pc(p1) == 5, pc(p2) == 5)

dnf_good = Or(cube1, cube2)
dnf_bad = Or(cube1, cube2, cube3)

# cube check
solver = Solver()
solver.add(typeok)
solver.add( And(dnf_good, Not(ii)) )
#solver.add( And(dnf_bad, Not(ii)) )
print("cube check: {}".format(unsat == solver.check()))
