...
var_subst = [(flag,flag_), (turn,turn_), (pc,pc_)]
...
#inv1_ = ForAll(p, Implies(
                    ...
                    #flag_(p) == True
                #))
#inv1_ = substitute(inv1, var_subst)
#inv1_ = substitute(inv1, [(pc,pc_)])
inv1oth = Implies(
                    Or(pc(p) == 3, pc(p) == 4, pc(p) == 5),
                    flag(p) == True
                )
inv1_ = ForAll(p, substitute(inv1oth, [(pc(p),pc_(p)),
                             (flag(p),flag_(p)),
                             (turn,turn_)]))
print("inv1_: {}".format(inv1_))
