...
%Furthermore, it is not clear that a protocol in EPR is guaranteed to have an inductive invariant that is also in EPR.  For any tool--such as SWISS and DistAI--that exclusively operates in EPR, or even \textit{works better} in EPR, the tool may experience issues if it is tough, or even impossible, for an inductive invariant to exist in EPR.  It may be the case that the protocol must be split into organized modules--which would be a natural consequence of developing an inductive invariant using the Ivy workflow--in order for \textit{any} inductive invariant to fit into EPR.  It is also unclear whether this is needed from running invariant inference tools.
